...
const mockedGetNfe = fiscalSvc.getNfe as jest.MockedFunction<typeof fiscalSvc.getNfe>;
const mockedCreate = fiscalSvc.createItemOverride as jest.MockedFunction<typeof fiscalSvc.createItemOverride>;

describe('NfeEditModal', () => {
  beforeEach(() => {
    mockedGetNfe.mockReset();
    mockedCreate.mockReset();
    (fiscalSvc.getNfeDivergencias as jest.MockedFunction<any>)?.mockReset?.();
    (fiscalSvc.applyItemOverride as jest.MockedFunction<any>)?.mockReset?.();
  });
  ...
  it('shows Refletir action for divergence and applies it', async () => {
    ...
    mockedGetNfe.mockResolvedValue({ data: mockNfe } as any);
    (fiscalSvc.getNfeDivergencias as jest.MockedFunction<any>).mockResolvedValue({ data: [{ item_id: 40, override_id: 123, quantidade_delta: '-2' }] } as any);
    (fiscalSvc.applyItemOverride as jest.MockedFunction<any>).mockResolvedValue({ data: {} } as any);

    render(<><NfeEditModal open={true} nfeId={4} onClose={() => {}} onSaved={() => {}} /><Toaster /></>);
  ...
  it('creates and applies override when no server divergence exists (reflect from stock mismatch)', async () => {
    ...
    mockedGetNfe.mockResolvedValue({ data: mockNfe } as any);
    (fiscalSvc.getNfeDivergencias as jest.MockedFunction<any>).mockResolvedValue({ data: [] } as any);
    // Mock product search to return a product with differing stock quantity
    jest.spyOn(produtosService, 'buscarSimples').mockResolvedValue([{ id: 99, codigo: 'CODE-Y', quantidade_estoque: 5 }] as any);

    mockedCreate.mockResolvedValue({ data: {} } as any);

    render(<><NfeEditModal open={true} nfeId={5} onClose={() => {}} onSaved={() => {}} /><Toaster /></>);
  ...
  it('renders items and saves overrides (non-confirmed NFe)', async () => {
    ...
    mockedGetNfe.mockResolvedValueOnce({ data: mockNfe } as any);
    mockedGetNfe.mockResolvedValueOnce({ data: updated } as any);
    mockedCreate.mockResolvedValue({ data: {} } as any);

    const onSaved = jest.fn();

    render(<><NfeEditModal open={true} nfeId={1} onClose={() => {}} onSaved={onSaved} /><Toaster /></>);
  ...
  it('does NOT create override when only formatting differs (99.50 vs 99.5)', async () => {
    ...
    render(<><NfeEditModal open={true} nfeId={8} onClose={() => {}} onSaved={() => {}} /><Toaster /></>);
  ...
  it('disables Refletir no Estoque when local unsaved edits exist or when NFe equals stock', async () => {
    ...
    jest.spyOn(produtosService, 'buscarSimples').mockResolvedValueOnce([{ id: 100, codigo: 'CODE-W', quantidade_estoque: 2, custo_medio: 50.00 }] as any);

    render(<><NfeEditModal open={true} nfeId={9} onClose={() => {}} onSaved={() => {}} /><Toaster /></>);
  ...
  it('shows confirmation dialog when NFe is confirmed and applies only after confirm', async () => {
    ...
    render(<><NfeEditModal open={true} nfeId={2} onClose={() => {}} onSaved={onSaved} /><Toaster /></>);
  ...
  it('handles 403 permission error gracefully', async () => {
    ...
    render(<><NfeEditModal open={true} nfeId={3} onClose={() => {}} onSaved={onSaved} /><Toaster /></>);
  ...
  it('calls onRefresh after saving overrides', async () => {
    ...
    render(<><NfeEditModal open={true} nfeId={2} onClose={() => {}} onSaved={() => {}} onRefresh={onRefresh} /><Toaster /></>);
