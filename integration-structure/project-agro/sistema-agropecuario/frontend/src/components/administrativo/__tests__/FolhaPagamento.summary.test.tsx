  ...
  it('loads and displays previous month summary automatically', async () => {
    ...
    mockedApi.get.mockImplementation((url: string) => {
      if (url.includes('/administrativo/funcionarios/')) {
        return Promise.resolve({ data: [{ id: 1, nome: 'João' }, { id: 2, nome: 'Maria' }] } as any);
      }
      if (url.includes('/administrativo/folha-pagamento/summary')) {
        return Promise.resolve({ data: { total_horas_extra_cost: 111.111, total_inss: 22.222, total_folha: 333.333 } } as any);
      }
