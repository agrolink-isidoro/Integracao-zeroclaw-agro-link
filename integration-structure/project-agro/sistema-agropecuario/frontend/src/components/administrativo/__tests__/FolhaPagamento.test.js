...
describe('FolhaPagamento', () => {
    ...
    it('generates preview and runs', async () => {
        ...
        mockedApi.post.mockResolvedValueOnce({ data: { id: 1, descricao: 'Folha 1/2026', valor_total: 1500, itens: [{ id: 1, funcionario: { id: 1, nome: 'João' }, salario_bruto: 1000, hora_extra: 50, dsr: 10, inss: 80, ir: 0, descontos_outro: 0, liquido: 970 }] } });
        // run endpoint
        mockedApi.post.mockResolvedValueOnce({ data: { status: 'executed', id: 1 } });
        // when run completes we fetch the persisted folha
        mockedApi.get.mockResolvedValueOnce({ data: [{ id: 1, nome: 'João' }] });
        // summary call for previous month (auto-loaded)
        mockedApi.get.mockResolvedValueOnce({ data: { total_horas_extra_cost: 0, total_inss: 0, total_folha: 0 } });
        mockedApi.get.mockResolvedValueOnce({ data: { id: 1, descricao: 'Folha 1/2026', periodo_mes: 1, periodo_ano: 2026, itens: [{ id: 1, funcionario: { id: 1, nome: 'João' }, salario_bruto: 1000, hora_extra: 50, dsr: 10, inss: 80, ir: 0, descontos_outro: 0, liquido: 970 }] } });
        render(_jsx(QueryClientProvider, { client: qc, children: _jsx(FolhaPagamento, {}) }));
        ...
        const fakeWindow = { document: { write: jest.fn(), close: jest.fn() }, focus: jest.fn(), print: jest.fn() };
    ...
    it('automatically includes DSR when hours > 0 and holidays > 0', async () => {
        mockedApi.get.mockResolvedValueOnce({ data: [{ id: 1, nome: 'João' }] });
        mockedApi.post.mockResolvedValueOnce({ data: { id: 2, descricao: 'Folha 1/2026', valor_total: 1500, itens: [{ id: 2, funcionario: { id: 1, nome: 'João' }, salario_bruto: 1000, hora_extra: 68.18, hora_extra_hours: 10, dsr: 5.23, inss: 80, ir: 0, descontos_outro: 0, liquido: 920 }] } });
        render(_jsx(QueryClientProvider, { client: qc, children: _jsx(FolhaPagamento, {}) }));
        await waitFor(() => expect(mockedApi.get).toHaveBeenCalledWith('/administrativo/funcionarios/'));
        ...
        expect(dsrInput.value).toBe('5.230');
        const perInclude = await screen.findByLabelText('Incluir DSR João');
        expect(perInclude.checked).toBe(true);
    });
    ...
    it('shows dias trabalhados for temporario and includes it in payload', async () => {
        mockedApi.get.mockResolvedValueOnce({ data: [{ id: 1, nome: 'TEMP', tipo: 'temporario', diaria_valor: 100 }] });
        mockedApi.post.mockResolvedValueOnce({ data: { id: 2, descricao: 'Folha 1/2026', valor_total: 100, itens: [{ id: 2, funcionario: { id: 1, nome: 'TEMP' }, salario_bruto: 300, hora_extra: 0, dsr: 0, inss: 0, ir: 0, descontos_outro: 0, liquido: 300 }] } });
        render(_jsx(QueryClientProvider, { client: qc, children: _jsx(FolhaPagamento, {}) }));
        await waitFor(() => expect(mockedApi.get).toHaveBeenCalledWith('/administrativo/funcionarios/'));
    ...
    it('handles string decimal fields when building report (no crash)', async () => {
        ...
        mockedApi.post.mockResolvedValueOnce({ data: { id: 3, descricao: 'Folha 1/2026', valor_total: 200, itens: [{ id: 3, funcionario: { id: 1, nome: 'Carlos' }, salario_bruto: '1000.50', hora_extra: '20.5', dsr: '5.25', inss: '80', ir: '0', descontos_outro: '0', liquido: '945.75' }] } });
        mockedApi.post.mockResolvedValueOnce({ data: { status: 'executed', id: 3 } });
        // mock GETs: funcionarios and later persisted folha
        mockedApi.get.mockImplementation((url) => {
            if (url === '/administrativo/funcionarios/')
                return Promise.resolve({ data: [{ id: 1, nome: 'Carlos' }] });
            if (url.startsWith('/administrativo/folha-pagamento/') && url.includes('/'))
                return Promise.resolve({ data: { id: 3, descricao: 'Folha 1/2026', periodo_mes: 1, periodo_ano: 2026, itens: [{ id: 3, funcionario: { id: 1, nome: 'Carlos' }, salario_bruto: '1000.50', hora_extra: '20.5', dsr: '5.25', inss: '80', ir: '0', descontos_outro: '0', liquido: '945.75' }] } });
        ...
        mockedApi.post.mockResolvedValueOnce({ data: { id: 3, descricao: 'Folha 1/2026', valor_total: 200, itens: [{ id: 3, funcionario: { id: 1, nome: 'Carlos' } }] } });
