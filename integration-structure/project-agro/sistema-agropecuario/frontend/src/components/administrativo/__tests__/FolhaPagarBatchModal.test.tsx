...
describe('FolhaPagarBatchModal', () => {
  ...
  beforeEach(() => { jest.resetAllMocks(); (mockedApi.get as jest.Mock).mockResolvedValue({ data: [{ id: 1, banco: 'B', agencia: '01', conta: '123' }] }); });

  it('renders rows and sends payload', async () => {
    mockedApi.post.mockResolvedValueOnce({ data: { results: [{ funcionario_id: 1, success: true, transfer_id: 1 }] } } as any);

    render(
      <QueryClientProvider client={qc}>
        <FolhaPagarBatchModal folhaId={5} items={[{ funcionario: { id: 1, nome: 'Alice' }, liquido: 150 }]} onClose={() => {}} />
      </QueryClientProvider>
    );
  ...
  it('validates pix requires pix_key or conta_destino', async () => {
    render(
      <QueryClientProvider client={qc}>
        <FolhaPagarBatchModal folhaId={5} items={[{ funcionario: { id: 2, nome: 'Bob' }, liquido: 120 }]} onClose={() => {}} />
      </QueryClientProvider>
    );
  ...
  it('reprocesses failed items', async () => {
    mockedApi.post.mockResolvedValueOnce({ data: { results: [{ funcionario_id: 1, success: false, error: 'Banco inválido' }] } } as any);
    mockedApi.post.mockResolvedValueOnce({ data: { results: [{ funcionario_id: 1, success: true, transfer_id: 3 }] } } as any);
