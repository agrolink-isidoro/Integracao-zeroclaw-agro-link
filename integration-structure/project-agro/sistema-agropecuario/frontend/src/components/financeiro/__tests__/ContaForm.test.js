...
describe('ContaForm', () => {
    ...
    it('alerts when saldo uses comma as decimal separator and does not call create', async () => {
        ...
        useApi.useApiCreate.mockReturnValue(mockCreate);
        ...
        useApi.useApiQuery.mockImplementation((key) => {
            if (Array.isArray(key) && key[0] === 'instituicoes')
                return { data: insts, isLoading: false };
            return { data: [], isLoading: false };
        });
        const { container } = render(_jsx(ContaForm, { onClose: () => { }, onSaved: () => { } }));
        ...
        const contaInput = container.querySelector('input[name="conta"]');
    ...
    it('selecting banco shows the selected label', async () => {
        ...
        useApi.useApiCreate.mockReturnValue({ mutateAsync: jest.fn() });
        render(_jsx(ContaForm, { onClose: () => { }, onSaved: () => { } }));
        // Open dropdown
        const dropdown = screen.getByText('Selecione instituição (nome ou código)');
        fireEvent.click(dropdown);
