...
describe('DespesaForm', () => {
  ...
  it('renders form, loads centros and submits', async () => {
    mockedApi.get.mockResolvedValueOnce({ data: [{ id: 1, codigo: 'C001', nome: 'Centro A' }] } as any);
    mockedApi.post.mockResolvedValueOnce({ data: { id: 5, titulo: 'X' } } as any);

    const onClose = jest.fn();

    render(
      <QueryClientProvider client={qc}>
        <DespesaForm onClose={onClose} />
      </QueryClientProvider>
    );
