...
describe('OperacaoForm (Empréstimo)', () => {
  ...
  it('sends numeric fields as numbers and includes data_primeiro_vencimento in payload', async () => {
    ...
    (useApi.useApiQuery as jest.Mock).mockImplementation((key: any) => {
      if (Array.isArray(key) && key[0] === 'clientes') return { data: [{ id: 1, nome: 'Cliente A' }], isLoading: false };
      return { data: [], isLoading: false };
    });

    const mutateAsync = jest.fn().mockResolvedValue({ id: 123 });
    (useApi.useApiCreate as jest.Mock).mockReturnValue({ mutateAsync });

    const onClose = jest.fn();
    const onSaved = jest.fn();

    const { container } = render(<OperacaoForm tipo="emprestimo" onClose={onClose} onSaved={onSaved} />);
    ...
    const taxaInput = taxaLabel.parentElement!.querySelector('input') as HTMLInputElement;
    ...
    const select = container.querySelector('select[name="beneficiario"]') as HTMLSelectElement;
