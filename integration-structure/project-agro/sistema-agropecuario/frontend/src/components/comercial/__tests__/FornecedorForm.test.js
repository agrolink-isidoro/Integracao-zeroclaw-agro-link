...
describe('FornecedorForm', () => {
    it('calls onSubmit with normalized data when form is valid', async () => {
        ...
        const { container } = render(_jsx(FornecedorForm, { isOpen: true, onClose: () => { }, onSubmit: onSubmit, loading: false, fornecedor: fornecedor }));
        // Use name attributes to find inputs (SelectDropdowns are custom components)
        const cpf = container.querySelector('input[name="cpf_cnpj"]');
        const razao = container.querySelector('input[name="razao_social"]');
        const categoria = container.querySelector('select[name="categoria_fornecedor"]');
        ...
        if (categoria)
            fireEvent.change(categoria, { target: { value: 'insumos' } });
        if (status)
            fireEvent.change(status, { target: { value: 'ativo' } });
        // Endereço
