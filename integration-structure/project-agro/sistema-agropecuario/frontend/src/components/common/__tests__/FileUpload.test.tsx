...
describe('FileUpload', () => {
  test('calls onFileSelect when a file is chosen and clears on resetKey change', async () => {
    ...
    const { container, rerender } = render(<FileUpload onFileSelect={onFileSelect} accept=".xml" multiple={false} label="Arquivo XML" />);
    ...
    rerender(<FileUpload onFileSelect={onFileSelect} accept=".xml" multiple={false} label="Arquivo XML" resetKey={1} />);
