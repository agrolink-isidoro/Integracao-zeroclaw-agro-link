  ...
  it('lists equipamentos and allows product search selection', async () => {
    mockedApi.get.mockImplementation((url: string) => {
      if (url.includes('/talhoes/')) {
        return Promise.resolve({ data: [{ id: 1, name: 'T1', area_hectares: 10 }] } as any);
      }
      if (url.includes('/agricultura/plantios/')) {
        return Promise.resolve({ data: [] } as any);
      }
      if (url.includes('/maquinas/equipamentos/')) {
        return Promise.resolve({ data: [{ id: 100, nome: 'Trator A', tipo_mobilidade: 'autopropelido' }, { id: 101, nome: 'Pulverizador 600L', tipo_implemento: 'pulverizador', largura_trabalho: 6 }] } as any);
      }
      if (url.includes('/estoque/produtos/')) {
        return Promise.resolve({ data: [{ id: 200, nome: 'Herbicida X', codigo: 'HX-1', dosagem_padrao: 2.5, unidade_dosagem: 'kg/ha' }] } as any);
      }
      if (url.includes('/agricultura/operacoes/tipos-por-categoria/')) {
        return Promise.resolve({ data: { tipos: [{ value: 'plantio', label: 'Plantio' }] } } as any);
      }
      return Promise.resolve({ data: [] } as any);
    });
    ...
    const dataInicio = container.querySelector('input[type="datetime-local"]') as HTMLInputElement;
    if (dataInicio) {
    ...
    const prodInput = await waitFor(() => 
      screen.findByPlaceholderText(/Buscar produto/i),
      { timeout: 3000 }
    );
    fireEvent.change(prodInput, { target: { value: 'Herb' } });

    // Wait for search results
    const resultItem = await waitFor(() => 
      screen.findByText(/Herbicida X/),
      { timeout: 3000 }
    );
    ...
    await waitFor(() => {
      expect((prodInput as HTMLInputElement).value).toMatch(/Herbicida X/);
    }, { timeout: 2000 });

    // The total usage should be calculated: dosagem 2.5 * area 10 = 25.000 kg
    const totalInput = await waitFor(() => 
      screen.findByLabelText(/Quantidade total/i),
      { timeout: 3000 }
    );
    expect((totalInput as HTMLInputElement).value).toMatch(/25(\.0+)?/);
    ...
    const dosInput = screen.getByLabelText(/Dosagem/i) as HTMLInputElement;
