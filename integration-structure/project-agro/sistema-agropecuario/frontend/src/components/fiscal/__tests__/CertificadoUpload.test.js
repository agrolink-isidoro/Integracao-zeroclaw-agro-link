...
describe('CertificadoUpload', () => {
    ...
    test('calls uploadCert on submit', async () => {
        fiscal.uploadCert.mockResolvedValue({ data: { id: 1 } });
        const { container, getByText } = render(_jsx(CertificadoUpload, {}));
    ...
    test('handles invalid file type error (server-side)', async () => {
        ...
        fiscal.uploadCert.mockRejectedValue({ response: { data: { error: 'invalid_file_type' } } });
        const { container, getByText } = render(_jsx(CertificadoUpload, {}));
        const input = container.querySelector('input[type="file"]');
