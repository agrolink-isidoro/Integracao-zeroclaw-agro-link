...
describe('NfeUpload', () => {
  ...
  test('calls uploadXml on submit and shows success', async () => {
    (fiscal.uploadXml as jest.Mock).mockResolvedValue({ data: { id: 1 } });

    const { container, getByText } = render(<NfeUpload />);
  ...
  test('displays bad_fields when validation error returned', async () => {
    (fiscal.uploadXml as jest.Mock).mockRejectedValue({ response: { data: { error: 'validation_error', bad_fields: [{ field: 'emitente_nome', message: 'excede' }] } } });

    const { container, getByText, findByText } = render(<NfeUpload />);
    const input: HTMLInputElement | null = container.querySelector('input[type="file"]');
    const file = new File(['<xml/>'], 'nota.xml', { type: 'text/xml' });
  ...
  test('clear button removes selected file and upload clears after success', async () => {
    ...
    await waitFor(() => expect((container.querySelector('input[type="file"]') as HTMLInputElement).value).toBe(''));
  });
});
