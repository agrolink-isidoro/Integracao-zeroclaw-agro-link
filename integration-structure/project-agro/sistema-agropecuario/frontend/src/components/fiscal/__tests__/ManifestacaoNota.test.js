...
describe('ManifestacaoNota', () => {
    ...
    beforeEach(() => {
        mockUseToast.mockReturnValue({ showSuccess: jest.fn(), showError: jest.fn(), showInfo: jest.fn() });
    });
    it('does not render when feature flag off', () => {
        // Simulate env variable not enabled
        import.meta.env = { VITE_FISCAL_MANIFESTACAO_ENABLED: 'false' };
        const { container } = render(_jsx(ManifestacaoNota, { nfeId: 1 }));
        expect(container).toBeEmptyDOMElement();
    });
    it('renders and submits when enabled and shows success', async () => {
        import.meta.env = { VITE_FISCAL_MANIFESTACAO_ENABLED: 'true' };
        const toastMocks = { showSuccess: jest.fn(), showError: jest.fn(), showInfo: jest.fn() };
        mockUseToast.mockReturnValue(toastMocks);
        fiscal.postManifestacao.mockResolvedValue({ data: {} });
        fiscal.listManifestacoesForNfe.mockResolvedValue({ data: { results: [] } });
        render(_jsx(ManifestacaoNota, { nfeId: 1 }));
    ...
    it('shows enqueued info when backend responds enqueued', async () => {
        ...
        const toastMocks = { showSuccess: jest.fn(), showError: jest.fn(), showInfo: jest.fn() };
        mockUseToast.mockReturnValue(toastMocks);
        fiscal.postManifestacao.mockResolvedValue({ status: 201, data: { enqueued: true } });
        fiscal.listManifestacoesForNfe.mockResolvedValue({ data: { results: [] } });
        render(_jsx(ManifestacaoNota, { nfeId: 1 }));
        const button = screen.getByText('Manifestar');
        fireEvent.click(button);
    ...
    it('shows error toast on failure', async () => {
        ...
        const toastMocks = { showSuccess: jest.fn(), showError: jest.fn(), showInfo: jest.fn() };
        mockUseToast.mockReturnValue(toastMocks);
        fiscal.postManifestacao.mockRejectedValue({ response: { data: { detail: 'Not allowed' } } });
        fiscal.listManifestacoesForNfe.mockResolvedValue({ data: { results: [] } });
        render(_jsx(ManifestacaoNota, { nfeId: 1 }));
        const button = screen.getByText('Manifestar');
        fireEvent.click(button);
