...
describe('OrdemServicoFormMaquinas', () => {
  ...
  it('submits create payload and calls API', async () => {
    ...
    const { container } = render(
      <QueryClientProvider client={queryClient}>
        <OrdemServicoForm onClose={onClose} onSuccess={onSuccess} />
      </QueryClientProvider>
    );
    ...
    // Select equipamento
    const equipamentoSelect = screen.getByLabelText(/Equipamento/i) as HTMLSelectElement;
    ...
    const equipamentoOption = screen.getByText('Trator A') as HTMLOptionElement;
    equipamentoOption.selected = true;
    fireEvent.change(equipamentoSelect);

    // Fill description
    const descricao = screen.getByLabelText(/Descrição do Problema/i) as HTMLTextAreaElement;
    ...
    const addBtn = screen.getByText(/Adicionar/i) as HTMLButtonElement;
    ...
    const quantidadeInput = container.querySelector('input[name="quantidade_insumo"]') as HTMLInputElement | null;
  ...
  it('sends existing ordemServico.nfes in the update payload', async () => {
    ...
    const ordem = { id: 99, equipamento: 1, descricao_problema: 'Teste', nfes: [123], insumos: [] } as any;

    const { container } = render(
      <QueryClientProvider client={queryClient}>
        <OrdemServicoForm ordemServico={ordem} onClose={onClose} onSuccess={onSuccess} />
      </QueryClientProvider>
    );
    ...
    const payload = mockedApi.put.mock.calls[mockedApi.put.mock.calls.length - 1][1] as any;
