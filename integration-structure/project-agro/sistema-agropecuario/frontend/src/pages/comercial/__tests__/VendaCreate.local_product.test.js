...
describe('VendaCreate local -> product', () => {
    beforeEach(() => {
        ComercialService.getClientes.mockResolvedValue([]);
        ComercialService.getLocais.mockResolvedValue([
            { id: 1, nome: 'Silo A' },
        ]);
        ComercialService.getProdutosByLocal.mockResolvedValue([
            { id: 5, nome: 'Milho' },
        ]);
    });
    it('loads products when local is selected and auto-selects single product', async () => {
        renderWithClient(_jsx(VendaCreate, {}));
        ...
        const localSelect = localLabel.parentElement?.querySelector('select[name="local_armazenamento"]');
        ...
        const produtoSelect = produtoLabel.parentElement?.querySelector('select[name="produto"]');
