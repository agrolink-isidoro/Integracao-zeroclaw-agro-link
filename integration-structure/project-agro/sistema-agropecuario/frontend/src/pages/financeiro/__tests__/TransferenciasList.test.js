...
describe('TransferenciasList', () => {
    it('renders empty state', async () => {
        useApi.useApiQuery.mockReturnValue({ data: [], isLoading: false });
        const { QueryClient, QueryClientProvider } = await import('@tanstack/react-query');
        const qc = new QueryClient();
        render(_jsx(QueryClientProvider, { client: qc, children: _jsx(TransferenciasList, {}) }));
    ...
    it('calls marcarTransferenciaSettled on click', async () => {
        ...
        useApi.useApiQuery.mockReturnValueOnce({ data: mockTransfers, isLoading: false });
        // second call (when opening modal) -> contas for transfer form, but not used in this test
        useApi.useApiQuery.mockReturnValueOnce({ data: [], isLoading: false });
    ...
    it('opens TransferForm modal on Nova Transferência click', async () => {
        ...
        useApi.useApiQuery.mockReturnValueOnce({ data: [{ id: 10, banco: 'B', conta: '123' }], isLoading: false });
        const { QueryClient, QueryClientProvider } = await import('@tanstack/react-query');
        const qc = new QueryClient();
        render(_jsx(QueryClientProvider, { client: qc, children: _jsx(TransferenciasList, {}) }));
