...
describe('Financeiro Dashboard - navigation', () => {
  it('renders module tabs and navigates to Financiamentos', async () => {
    ...
    mocked.getResumoFinanceiro.mockResolvedValue(sample as ResumoFinanceiro);
    ...
    render(
      <QueryClientProvider client={qc}>
        <MemoryRouter initialEntries={["/financeiro/dashboard"]}>
          <Routes>
            <Route path="/financeiro/dashboard" element={<ModuleTabs tabs={tabs} />} />
            <Route path="/financeiro/financiamentos" element={<div>Financiamentos Page</div>} />
          </Routes>
        </MemoryRouter>
      </QueryClientProvider>
    );
...
describe('Financeiro Dashboard', () => {
  it('renders summary numbers and charts', async () => {
    ...
    const withinVenc = within(cardBody as HTMLElement);
  ...
  it('passes selected date to API when updated', async () => {
    ...
    const input = await screen.findByLabelText('Data referência') as HTMLInputElement;
