...
describe('EmprestimoCreate', () => {
  it('submits form and calls onSuccess when provided', async () => {
    mocked.createEmprestimo.mockResolvedValue({ id: 222 } as any);
    const onSuccess = jest.fn();

    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    render(<QueryClientProvider client={qc}><MemoryRouter><EmprestimoCreate onSuccess={onSuccess} /></MemoryRouter></QueryClientProvider>);
