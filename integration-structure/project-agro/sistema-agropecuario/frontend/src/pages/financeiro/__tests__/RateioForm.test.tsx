...
describe('RateioForm', () => {
  ...
  it('validates area driver requires talhoes', async () => {
    ...
    const { container } = render(<MemoryRouter><RateioForm /></MemoryRouter>);
  ...
  it('submits payload when valid', async () => {
    (useApiQuery as jest.Mock).mockImplementation((...args: any[]) => {
      // return sensible data per endpoint
      const key = args[0] && args[0][0];
      if (key === 'centros-custo') return { data: [{ id: 5, codigo: 'C1', nome: 'Centro 1' }], isLoading: false };
      if (key === 'plantios') return { data: [{ id: 2, cultura_nome: 'Soja' }], isLoading: false };
