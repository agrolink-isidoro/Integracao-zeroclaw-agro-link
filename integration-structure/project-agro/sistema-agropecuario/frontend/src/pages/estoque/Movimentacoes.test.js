...
describe('MovimentacoesPage', () => {
    ...
    test('renders list and applies product autocomplete filter', async () => {
        produtosService.buscarSimples.mockResolvedValue([{ id: 1, nome: 'Produto A', codigo: 'P-A' }]);
        movimentacoesService.listarStatements.mockResolvedValue({ count: 1, results: [{ id: 10, produto: 1, produto_nome: 'Produto A', tipo: 'entrada', quantidade: 5, data_movimentacao: '2025-01-01T00:00:00Z' }] });
        render(_jsx(MovimentacoesPage, {}));
        ...
        await waitFor(() => expect(movimentacoesService.listarStatements).toHaveBeenCalled());
