...
describe('LocalArmazenagemForm', () => {
    ...
    it('shows saca_60kg option and submits create payload', async () => {
        // Mock fazendas query
        useApiQuery.mockReturnValue({ data: [{ id: 1, name: 'Fazenda A' }], isLoading: false });
        const mutateAsync = jest.fn().mockResolvedValue({});
        useApiCreate.mockReturnValue({ mutateAsync });
        useApiUpdate.mockReturnValue({ mutateAsync: jest.fn() });
        const onSuccess = jest.fn();
        const { container } = render(_jsx(LocalArmazenagemForm, { onSuccess: onSuccess }));
        // Ensure unit option exists (select by name)
        const unidadeSelect = container.querySelector('select[name="unidade_capacidade"]');
        ...
        const capacidadeInput = container.querySelector('input[name="capacidade_total"]');
    ...
    it('submits update payload when editing an existing local', async () => {
        ...
        useApiCreate.mockReturnValue({ mutateAsync: jest.fn() });
        useApiUpdate.mockReturnValue({ mutateAsync: updateMutate });
        const onSuccess = jest.fn();
        const local = { id: 5, nome: 'Silo Velho', tipo: 'silo', capacidade_total: 1000, unidade_capacidade: 'kg', fazenda: 1, ativo: true };
        const { container } = render(_jsx(LocalArmazenagemForm, { local: local, onSuccess: onSuccess }));
