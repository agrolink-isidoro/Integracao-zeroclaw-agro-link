...
describe('LocaisArmazenagemList', () => {
    ...
    it('renders list and opens modal (kg case)', async () => {
        mockedApi.get.mockResolvedValue({ data: [{ id: 1, nome: 'Silo A', tipo: 'silo', capacidade_total: 1000, unidade_capacidade: 'kg', fazenda: 1, ativo: true }] });
        render(_jsx(QueryClientProvider, { client: qc, children: _jsx(LocaisArmazenagemList, {}) }));
    ...
    it('shows saca_60kg friendly label in list', async () => {
        mockedApi.get.mockResolvedValue({ data: [{ id: 2, nome: 'Depósito Saca', tipo: 'armazem', capacidade_total: 120, unidade_capacidade: 'saca_60kg', fazenda: 1, ativo: true }] });
        render(_jsx(QueryClientProvider, { client: qc, children: _jsx(LocaisArmazenagemList, {}) }));
