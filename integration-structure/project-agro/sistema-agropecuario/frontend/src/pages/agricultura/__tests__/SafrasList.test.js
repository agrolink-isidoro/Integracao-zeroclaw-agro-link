...
describe('SafrasList', () => {
    ...
    it('renders safra and handles manejo with null cost without crashing', async () => {
        mockedApi.get.mockImplementation((url) => {
            if (url.includes('agricultura/plantios/')) {
                return Promise.resolve({ data: [{ id: 1, nome_safra: 'Safra Teste', talhoes: [1], talhoes_info: [{ id: 1, nome: 'T1', area_hectares: 10 }], cultura_nome: 'Milho', data_plantio: '2025-09-24', area_total_ha: 10, status: 'em_andamento' }] });
            }
            if (url.includes('agricultura/manejos/')) {
                return Promise.resolve({ data: [{ id: 1, plantio: 1, tipo: 'adubacao', data_manejo: '2025-10-01', custo: null }] });
            }
        ...
        render(_jsx(QueryClientProvider, { client: qc, children: _jsx(SafrasList, {}) }));
    ...
    it('handles paginated plantios response without crashing', async () => {
        mockedApi.get.mockImplementation((url) => {
            if (url.includes('agricultura/plantios/')) {
                return Promise.resolve({ data: { count: 1, results: [{ id: 10, nome_safra: 'Safra Paginated', talhoes: [1], talhoes_info: [{ id: 1, nome: 'T1', area_hectares: 5 }], cultura_nome: 'Soja', data_plantio: '2025-09-24', area_total_ha: 5, status: 'planejado' }] } });
            }
            if (url.includes('agricultura/manejos/')) {
                return Promise.resolve({ data: [] });
            }
