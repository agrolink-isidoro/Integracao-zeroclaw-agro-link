...
const mockListManifestacoesForNfe = fiscalService.listManifestacoesForNfe;
describe('useManifestacaoPolling', () => {
    ...
    it('deve iniciar polling quando startPolling é chamado', () => {
        // ARRANGE
        const { result } = renderHook(() => useManifestacaoPolling({ nfeId: 123 }));
        // ACT
        act(() => {
    ...
    it('deve parar polling quando status muda para "sent"', async () => {
        ...
        const { result } = renderHook(() => useManifestacaoPolling({
            nfeId: 123,
            onStatusChange,
            intervalMs: 1000
        }));
    ...
    it('deve lidar com erros de API sem quebrar polling', async () => {
        ...
        // ASSERT
        expect(consoleWarnSpy).toHaveBeenCalledWith('Erro ao verificar status da manifestação:', expect.any(Error));
