...
export function isValidCPF(cpf: string): boolean {
  ...
  if (/^(\d)\1{10}$/.test(digits)) return false; // Verifica se todos os dígitos são iguais
...
export function isValidCNPJ(cnpj: string): boolean {
  ...
  if (/^(\d)\1{13}$/.test(digits)) return false; // Verifica se todos os dígitos são iguais
 ...
 */
export function calcularDiasDiferenca(dataInicio: string, dataFim: string): number {
