...
import type { EffectivePermissions, RBACModule, ModulePermissionFlags } from '../types/rbac';

const PERMISSIONS_KEY = 'sistema_agro_permissions';

function getStoredPermissions(): EffectivePermissions | null {
  try {
...
function setStoredPermissions(perms: EffectivePermissions | null) {
  try {
...
export function useRBAC() {
  ...
  const isSuperuser = useMemo(() => {
    ...
    const isOwnerLevel = (u: any) =>
      !!u?.is_superuser ||
      u?.is_staff === true ||
      (typeof u?.cargo === 'string' &&
        ['proprietário', 'proprietario', 'owner', 'admin'].includes(
          u.cargo.trim().toLowerCase()
        ));

    // Check from live user object first
    if (user && isOwnerLevel(user)) return true;
    ...
    try {
      ...
      if (raw) {
        const stored = JSON.parse(raw);
        if (isOwnerLevel(stored)) return true;
      }
    } catch { /* ignore */ }
  ...
  const hasPermission = useCallback(
    (module: RBACModule, level: keyof ModulePermissionFlags = 'can_view'): boolean => {
  ...
  const canView = useCallback(
    (module: RBACModule): boolean => hasPermission(module, 'can_view'),
    [hasPermission]
  );
  ...
  const canEdit = useCallback(
    (module: RBACModule): boolean => hasPermission(module, 'can_edit'),
    [hasPermission]
  );
  ...
  const canRespond = useCallback(
    (module: RBACModule): boolean => hasPermission(module, 'can_respond'),
    [hasPermission]
  );
  ...
  const isAdmin = useMemo(
    () => isSuperuser || hasPermission('user_management', 'can_view'),
   ...
   */
  const visibleModules = useMemo((): RBACModule[] => {
    if (isSuperuser) {
      return [
    ...
    for (const [mod, flags] of Object.entries(permissions)) {
      if (flags?.can_view && mod !== 'dashboard') {
        modules.push(mod as RBACModule);
      }
    }
   ...
   */
  const updatePermissions = useCallback((newPerms: EffectivePermissions) => {
