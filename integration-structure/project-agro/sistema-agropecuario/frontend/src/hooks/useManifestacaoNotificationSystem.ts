...
interface ManifestacaoNotification {
  nfeId: number;
  status: string;
  chaveAcesso: string;
  timestamp: Date;
...
interface NotificationSystemOptions {
  ...
  intervalMs?: number;
  maxRetries?: number;
  onNotification?: (notification: ManifestacaoNotification) => void;
}

export const useManifestacaoNotificationSystem = ({
  enabled = false,
  intervalMs = 10000, // 10 segundos - menos frequente que o polling individual
  maxRetries = 3,
  onNotification
}: NotificationSystemOptions) => {
  const [isActive, setIsActive] = useState(false);
  const [notifications, setNotifications] = useState<ManifestacaoNotification[]>([]);
  ...
  const checkPendingManifestacoes = async (): Promise<ManifestacaoNotification[]> => {
    try {
  ...
  const removeNotification = (nfeId: number) => {
