...
import type { CustomUser, LoginData, TenantInfo } from '../types'
...
export function setStoredUser(user: CustomUser | null) {
  try {
...
export default function useAuth() {
  const [user, setUser] = useState<CustomUser | null>(() => getStoredUser())
  const [loading, setLoading] = useState<boolean>(true)

  const setTokensAndHeader = useCallback((tokens: Tokens | undefined) => {
    if (!tokens) return
    ...
    const init = async () => {
      ...
      if (tokens?.access) {
        ...
        try {
          ...
          if (!hasValidUser || (profile as any)?.code === 403) {
  ...
  const login = useCallback(async (username: string, password: string) => {
    try {
      ...
      const { access, refresh, user: userData } = resp as any
      ...
      const tenantInfo = (userData as any)?.tenant_info || null
