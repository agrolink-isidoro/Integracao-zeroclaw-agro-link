...
class ConciliacaoService {
    ...
    async getBankStatementImports(conta) {
        try {
    ...
    async getBankStatementImportById(id) {
        try {
     ...
     */
    async conciliarImportacao(importId) {
        try {
    ...
    async getItensExtrato(params) {
        try {
            ...
            if (params?.conta_bancaria)
                queryParams.append('conta_bancaria', String(params.conta_bancaria));
            if (params?.conciliado !== undefined)
                queryParams.append('conciliado', String(params.conciliado));
            if (params?.tipo)
                queryParams.append('tipo', params.tipo);
            if (params?.data)
                queryParams.append('data', params.data);
    ...
    async getItensPendentes(contaBancaria) {
        try {
     ...
     */
    async conciliarManual(itemId, vencimentoId) {
        try {
     ...
     */
    async conciliarComTransferencia(itemId, transferenciaId) {
        try {
            const response = await api.post(`/financeiro/itens-extrato/${itemId}/conciliar_manual/`, {
