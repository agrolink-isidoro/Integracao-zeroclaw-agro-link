...
else if (typeof window !== 'undefined' && window.VITE_API_BASE) {
    // Optional global set by dev server if needed
    baseURL = window.VITE_API_BASE;
}
...
api.interceptors.request.use((config) => {
    ...
    try {
        if (cfg.baseURL && cfg.url)
            fullUrl = new URL(cfg.url, cfg.baseURL).toString();
        else if (typeof window !== 'undefined' && cfg.url)
            fullUrl = new URL(cfg.url, window.location.origin).toString();
    }
    catch (e) {
    ...
    if (token) {
        ...
        console.debug('[API] Authorization header set:', cfg.headers.Authorization?.substring(0, 30) + '...');
    }
    else {
    ...
    cfg.headers = cfg.headers || {};
    if (tenantInfo?.id) {
        cfg.headers['X-Tenant-ID'] = tenantInfo.id;
        ...
        delete cfg.headers['X-Tenant-ID'];
...
const responseFulfilled = (response) => {
    ...
    try {
        ...
        if (data && typeof data === 'object' && data.code === 403) {
            ...
            try {
                const events = window.__apiForbiddenEvents = window.__apiForbiddenEvents || [];
...
;
api.__responseFulfilled = responseFulfilled;
...
if (typeof window !== 'undefined') {
    ...
    window.__apiForbiddenEvents = window.__apiForbiddenEvents || [];
