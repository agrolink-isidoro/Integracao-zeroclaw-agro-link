...
class RateioModelAndServiceTests(TestCase):
    def setUp(self):
        self.tenant = Tenant.objects.create(nome='test_tenant_rateio_new_fields', slug='test-tenant-rateio-new-fields')
        self.user = User.objects.create_user(username='u', password='p', is_staff=False, tenant=self.tenant)
        self.prop = Proprietario.objects.create(nome='P', cpf_cnpj='000', tenant=self.tenant)
        self.fazenda = Fazenda.objects.create(proprietario=self.prop, name='F', matricula='M', tenant=self.tenant)
        self.area = Area.objects.create(proprietario=self.prop, fazenda=self.fazenda, name='A', geom='POINT(0 0)', tenant=self.tenant)
        self.talhao = Talhao.objects.create(area=self.area, name='T1', area_size=10, tenant=self.tenant)
        self.centro, _ = CentroCusto.objects.get_or_create(codigo='ADM', defaults={'nome':'Administrativo', 'tenant': self.tenant})

    def test_driver_area_requires_talhao(self):
        r = RateioCusto.objects.create(titulo='R1', valor_total=Decimal('100.00'), criado_por=self.user, tenant=self.tenant)
    ...
    def test_centrocusto_required_for_despesa_adm(self):
        r = RateioCusto.objects.create(titulo='R2', valor_total=Decimal('200.00'), criado_por=self.user, tenant=self.tenant)
    ...
    def test_approval_creates_vencimento_for_despesa_adm_but_not_operacional(self):
        ...
        d = DespesaAdministrativa.objects.create(titulo='D1', valor=Decimal('500.00'), data=timezone.now().date(), centro=self.centro, tenant=self.tenant)
        ...
        r2 = RateioCusto.objects.create(titulo='R-Op', valor_total=Decimal('120.00'), criado_por=self.user, destino='operacional', tenant=self.tenant)
