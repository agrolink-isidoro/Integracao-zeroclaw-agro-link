...
class ProdutoUltimoPrecoTests(APITestCase):
    def setUp(self):
        self.tenant = Tenant.objects.create(
            nome='test_tenant_estoque_views_extra',
            slug='test-tenant-estoque-views-extra'
        )
        self.user = User.objects.create_user(username='u1', tenant=self.tenant)
        self.prop = Proprietario.objects.create(nome='P', cpf_cnpj='123', tenant=self.tenant)
        self.faz = Fazenda.objects.create(proprietario=self.prop, name='F', matricula='M1', tenant=self.tenant)
        self.prod = Produto.objects.create(codigo='P1', nome='Diesel', unidade='L', quantidade_estoque=Decimal('100'), tenant=self.tenant)
    ...
    def test_ultimo_preco_returns_last_entry(self):
        MovimentacaoEstoque.objects.create(produto=self.prod, tipo='entrada', quantidade=10, valor_unitario=Decimal('4.25'), tenant=self.tenant)
        MovimentacaoEstoque.objects.create(produto=self.prod, tipo='entrada', quantidade=20, valor_unitario=Decimal('4.5'), tenant=self.tenant)
        res = self.client.get('/api/estoque/produto-ultimo-preco/?produto_id=%s' % self.prod.pk)
        self.assertEqual(res.status_code, 200)
    ...
    def test_search_movimentacoes_by_produto_name(self):
        ...
        MovimentacaoEstoque.objects.create(produto=self.prod, tipo='entrada', quantidade=5, valor_unitario=Decimal('4.75'), tenant=self.tenant)
