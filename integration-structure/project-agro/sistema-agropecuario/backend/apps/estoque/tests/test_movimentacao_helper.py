...
class MovimentacaoHelperTests(TestCase):
    def test_create_entrada_sets_snapshots_and_statement(self):
        ...
        p = Produto.objects.create(codigo='P-ENT', nome='Produto Entrada', unidade='kg', quantidade_estoque=Decimal('10'))

        m = create_movimentacao(produto=p, tipo='entrada', quantidade=Decimal('5'), valor_unitario=Decimal('2.50'), criado_por=user)
    ...
    def test_create_saida_blocks_negative(self):
        ...
        p = Produto.objects.create(codigo='P-SAI', nome='Produto Saida', unidade='kg', quantidade_estoque=Decimal('2'))

        with self.assertRaises(ValidationError):
            create_movimentacao(produto=p, tipo='saida', quantidade=Decimal('5'), criado_por=user)
